...
public class InfoBillDAO {
    ...
    public double getDoanhThuThang1() {
        double doanhThu = 0;
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Book.price * InfoBill.amountPay)as 'tongtien' FROM Bill INNER JOIN InfoBill on Bill.idBill = InfoBill.idBill INNER JOIN Book on InfoBill.idBook = Book.idBook where strftime('%m', Bill.dop) = strftime('%m','now','-7 months') GROUP BY InfoBill.idBook)tmp ";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
            doanhThu = c.getDouble(0);
    ...
    public double getDoanhThuThang2() {
        double doanhThu = 0;
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Book.price * InfoBill.amountPay)as 'tongtien' FROM Bill INNER JOIN InfoBill on Bill.idBill = InfoBill.idBill INNER JOIN Book on InfoBill.idBook = Book.idBook where strftime('%m', Bill.dop) = strftime('%m','now','-6 months') GROUP BY InfoBill.idBook)tmp ";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
            doanhThu = c.getDouble(0);
    ...
    public double getDoanhThuThang3() {
        double doanhThu = 0;
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Book.price * InfoBill.amountPay)as 'tongtien' FROM Bill INNER JOIN InfoBill on Bill.idBill = InfoBill.idBill INNER JOIN Book on InfoBill.idBook = Book.idBook where strftime('%m', Bill.dop) = strftime('%m','now','-5 months') GROUP BY InfoBill.idBook)tmp ";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
            doanhThu = c.getDouble(0);
    ...
    public double getDoanhThuThang4() {
        double doanhThu = 0;
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Book.price * InfoBill.amountPay)as 'tongtien' FROM Bill INNER JOIN InfoBill on Bill.idBill = InfoBill.idBill INNER JOIN Book on InfoBill.idBook = Book.idBook where strftime('%m', Bill.dop) = strftime('%m','now','-4 months') GROUP BY InfoBill.idBook)tmp ";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
            doanhThu = c.getDouble(0);
    ...
    public double getDoanhThuThang5() {
        double doanhThu = 0;
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Book.price * InfoBill.amountPay)as 'tongtien' FROM Bill INNER JOIN InfoBill on Bill.idBill = InfoBill.idBill INNER JOIN Book on InfoBill.idBook = Book.idBook where strftime('%m', Bill.dop) = strftime('%m','now','-3 months') GROUP BY InfoBill.idBook)tmp ";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
            doanhThu = c.getDouble(0);
    ...
    public double getDoanhThuThang6() {
        double doanhThu = 0;
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Book.price * InfoBill.amountPay)as 'tongtien' FROM Bill INNER JOIN InfoBill on Bill.idBill = InfoBill.idBill INNER JOIN Book on InfoBill.idBook = Book.idBook where strftime('%m', Bill.dop) = strftime('%m','now','-2 months') GROUP BY InfoBill.idBook)tmp ";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
            doanhThu = c.getDouble(0);
    ...
    public double getDoanhThuThang7() {
        double doanhThu = 0;
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Book.price * InfoBill.amountPay)as 'tongtien' FROM Bill INNER JOIN InfoBill on Bill.idBill = InfoBill.idBill INNER JOIN Book on InfoBill.idBook = Book.idBook where strftime('%m', Bill.dop) = strftime('%m','now','-1 months') GROUP BY InfoBill.idBook)tmp ";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
        while (c.isAfterLast() == false) {
            doanhThu = c.getDouble(0);
    ...
    public double getDoanhThuThang8() {
        double doanhThu = 0;
        String sSQL = "SELECT SUM(tongtien) from (SELECT SUM(Book.price * InfoBill.amountPay)as 'tongtien' FROM Bill INNER JOIN InfoBill on Bill.idBill = InfoBill.idBill INNER JOIN Book on InfoBill.idBook = Book.idBook where strftime('%m', Bill.dop) = strftime('%m', 'now') GROUP BY InfoBill.idBook)tmp ";
        Cursor c = db.rawQuery(sSQL, null);
        c.moveToFirst();
